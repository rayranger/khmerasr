...
db.session.add_all([speaker_1, speaker_2])
db.session.commit()
...
db.session.add_all([record_1, record_2, record_3, record_4])
db.session.commit()

# create record config
record_config_1 = record_config.RecordConfig(duration=5, frame_per_buffer=1000, channel=1, rate=1000, created_at=TODAY_DATE_TIME, updated_at=TODAY_DATE_TIME)

db.session.add(record_config_1)
db.session.commit()
